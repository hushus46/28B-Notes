...
\begin{prop}[title= Coset space is a ring]
	...
	\end{align*}
\end{prop}
\Note If $I$ is understood, we will often write $\obar{r}$ for $r+I$, e.g
\[(r+I)+(s+I)=(r+s)+I\]
becomes
...
\end{proof}
\newpage
\Obs that	$R/I$ consists of the equivalence classes in $R$ of the equivalence relation given by
...
\begin{proof}[Proof of Prop 4.1]~\\
	...
	&\obar{a}\rdot (\obar{b}+\obar{c})=\obar{a}\rdot (\obar{b+c})=\obar{a\rdot (b+c)}=\obar{a\rdot b+a\rdot c}=\obar{ab}+\obar{ac}=\obar{a}\rdot \obar{b}+\obar{a}\rdot \obar{c} \qedhere
	\end{align*}
\end{proof}
\begin{dfn}[title=Ideal]
	...
	\end{enumerate}
\end{dfn}
\Obs that if $f:R\to S$ is a ring homomorphism then $\Ker f$ is an ideal in $R$.\\
\Note We may define $R/I$ for \textbf{any} ideal $I\subset R$, whether or not $I=\Ker f$ for some ring homomorphism $f:R\to S$.
\newpage
...
\end{example}
\Note $0\in I$ because it has \textbf{no} terms with non-zero coefficient.\\
...
\Obs that $R/I$ has zero divisors: $\obar{x}\rdot \obar{X}=\obar{X^2}=\obar{0}$.
\begin{example}
