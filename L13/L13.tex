...
\begin{lem}[title = Gauss' Lemma,label=gauss]
...
\begin{proof}
	...
	\end{align*}
	Furthermore,
	\[\divs{q_i}{d}\implies \obar{dp(X)}=\obar{0} \in (R/(q_i))[X] \implies \obar{a'(X)}\rdot \obar{ b'(X)} =\obar{0}\]
...
\begin{proof}
	...
	The other direction is \hyperref[lem:gauss]{Gauss' Lemma}.
\end{proof}
